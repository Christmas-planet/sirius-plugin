...
- `pstack`: poteto-agentが自分の手順の中で独立レビュー（`interrogate` や別モデルの確認）を行い、その結果を返す。Siriusはレビューをやり直さない。返ってきた結果から、レビューしたheadのSHA、レビューしたモデル、未解決の指摘の有無を確かめる。PR対応では、レビューが照合したコメントがそのPRのもの（コメントの `pull_request_url` がそのPR番号）だったことも確かめる。同じリポジトリの別PRを並行して扱うと取り違えが起きる。別PRのコメントで行ったレビューは判定として扱わず、正しいコメントIDを渡してやり直させる。未解決の指摘が残っているのに完了と報告されたら、同じ仕事を新しいpoteto-agentに続きとして渡す（前回の要約を信じて閉じない）。
