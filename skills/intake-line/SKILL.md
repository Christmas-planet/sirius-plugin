---
name: intake-line
description: ~/.sirius/repos/*.yamlと~/.sirius/workspaces/*.yamlに列挙されたLINEチャットを、ネイティブのmacOS LINEアプリで凍結された時間窓だけ読み、未解決の依頼を（workspaceのソースなら影響するリポジトリごとに振り分けて）create-issue経由で重複排除しつつGitHub Issueにし、reply.modeに従って依頼者へ自然な返信を下書きまたは送信する。Sirius runスキルから呼ばれ、Computer Useのあるローカルセッションが必要。
...
LINEは読み取り専用のまま扱う: リアクション、送信取消、転送、通話、ダウンロードは一切しない。入力欄への入力と送信は、依頼者への返信だけの例外（下の「reply.modeについて」を見る）。
...
文面は[返信契約](../../references/reply-contract.md)に従って書く。LINEは相手がクライアントや知人であることが多いので、GitHubのURLや「Issue」「PR」といった言葉は原則入れず、会話の続きとして短く返す。LINEには下書きを送る仕組みがない。

- `draft`: 送信せず、返信文をIssueかこの実行のレポートに書く。
- `send`: そのチャットを開き直し、送信直前にヘッダーがチャット名と完全一致することを確認してから、入力欄に文面を入力する。改行はEnterではなく改行用の操作で入れ、途中で送信されないようにする。送信前に入力欄の文字を読み取り、書いた文面と一致することを確かめてから送信ボタンを押す。送信後、画面上でそのメッセージが送られたことを確認し、チャット名と時刻を実行レポートに記録する。どこかの確認が取れなければ送信せず、入力欄を空に戻して `draft` と同じ扱いにし、レポートに書く。
